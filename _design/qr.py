...
eyes get rounded rings with a lime centre; a 9-module square in the middle is
left clear for the W badge the artboard lays on top. Error correction H keeps
...
def build_svg(m, style, light, eye, unit=20):
    ...
    # finder eyes: a rounded ring, one module thick, and a rounded 3x3 centre
    for (fx, fy) in [(0, 0), (n - 7, 0), (0, n - 7)]:
        ...
        parts.append(f'<rect x="{ox + 2 * unit}" y="{oy + 2 * unit}" width="{3 * unit}" height="{3 * unit}" rx="{unit * 0.9}" fill="{eye}"/>')
    parts.append('</svg>')
    return "\n".join(parts), n
