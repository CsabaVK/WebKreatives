...
One template, two outputs, per client:
...
Output: `tools/onboarding/out/<slug>/onboarding-<slug>.pdf` and `index.html`
...
| `--price` / `--hosting` | `€ —` / `€ 25` | build price, monthly fee |
...
| `--lang` | `en` | language the interactive page opens in |
...
Cover (bilingual, pick a language) → per language: 1 what happens now ·
2 what you get · 3 what we need from you · 4 where is your domain (provider
menu) · provider pages (Hostinger, GoDaddy, Cloudflare, Wix, Squarespace,
TransIP, other/not sure) · 5 what you never have to do · 6 what you own.
...
`template.html` is the single source. Copy lives inline per language
